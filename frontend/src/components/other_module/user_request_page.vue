<template>
    <div class="blogs">
        ...
        <div class="wrapper">
            <div class="top">
                <div class="title"><h1>Request The Admin for Priivileged Status</h1></div>
            </div>
            <div class="abc">
                <main class="container">
                    <form @submit.prevent="send_request">
                      ...
                      <button class="btn btn-primary w-100 py-2" type="submit" fdprocessedid="e845cp">Submit Request</button>
            
                    </form>
                    <p>Don't Wanna Privileged Status? &nbsp; &nbsp; &nbsp; &nbsp;<button class="add" @click="back"> Go Back </button></p>
                </main>
            </div>
    ...
    methods:{
        ...
        async send_request(){
          ...
          if(response.status==200){
            alert('Requst Sent wait for Admin\'s Approval!')
            this.user_id=JSON.parse(localStorage.getItem("info")).id
            this.$router.push({path:`/user_dashboard/${this.user_id}`})
          }
          else{
