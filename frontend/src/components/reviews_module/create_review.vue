<template>
    <div class="blogs">
        ...
        <div class="wrapper">
            <div class="top">
                <div class="title"><h1>Create a Review</h1></div>
            </div>
            <div class="abc">
                <main class="container">
                    <form @submit.prevent="add_blog">
...
export default {
    ...
    methods:{
        ...
        async add_blog(){
            try{
              this.id=JSON.parse(localStorage.getItem("info")).id
                const response=await axios.post(`http://127.0.0.1:5000/api/create_review/${this.$route.params.id}/${this.id}`,
                    JSON.stringify({
                    title:this.name,
                    rating:this.title,
