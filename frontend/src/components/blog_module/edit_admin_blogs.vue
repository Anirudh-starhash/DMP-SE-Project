...
export default {
    ...
    async mounted() {
      ...
      if (!access_token) {
        ...
        try {
          ...
          if (r.status === 200) {
            const response=await axios.get(`http://127.0.0.1:5000/api/getAdminInfoBlog/${this.$route.params.id}`);
            if(response.status==200){
                this.title=response.data.blog['admin_blog_subtitle'];
                this.name=response.data.blog['admin_blog_name'];
                this.body=response.data.blog['admin_blog_content']
    ...
    methods:{
        ...
        async edit_blog(){
            try{
                const response=await axios.put(`http://127.0.0.1:5000/api/edit_admin_blob/${this.$route.params.id}`,
                    JSON.stringify({
                    title:this.name,
                    subtitle:this.title,
                    body:this.body,
